...
| `rooms` | 牌局记账房间 | **所有用户可读**，仅创建者可写 |
| `records` | 牌局记账流水 | 所有用户可读，仅创建者可写 |
...
打麻将 / 打扑克不带现金时的房间记账工具。核心只有一条操作：**点人 → 点收/付 → 填金额**，任何牌型都能拆成这个动作；自摸、点炮、平摊只是自动帮你点好几下。

### 页面

| 页面 | 说明 |
|---|---|
| `pages/ledger/ledger` | 入口：创建房间 / 输房号加入 / 最近的牌局 |
| `pages/ledger/create` | 建房：起名字、加人（不填名字默认「玩家N」），支持 3/4/5 人一键切换 |
| `pages/ledger/room` | 房间：余额网格、四种记账模式、撤销、流水、散场结算 |

### 记账模式（`utils/settle.js`，纯函数已单测）

| 模式 | 含义 | 举例（4 人局） |
|---|---|---|
| 单笔 | 选一个人，收/付一笔 | 阿强收 30 |
| 自摸 | 选赢家，填「每家付多少」 | 每家 20 → 赢家 +60，其余各 −20 |
| 点炮 | 先点赢家，再点放炮者 | 50 → 赢家 +50，放炮者 −50 |
| 一人收全场 | 选收钱的人，填总金额 | 100 → 收 100，其余三人 −34/−33/−33 |

> 不能整除时余数按顺序摊给前面的人，**保证总额严格为零**。

### 结算算法

散场时按「最大债权人 ↔ 最大债务人」贪心配对，比人人都找赢家少很多笔现金。实测阿强 +120、小李 −40、王哥 −80 只需 **2 笔**（王哥给阿强 80、小李给阿强 40）。

### 数据模型

```js
// rooms
{ code, name, ownerOpenid, members: [{ id, name }], balances: { memberId: Number }, status: 'open'|'closed', entrySeq, createdAt, updatedAt }
// records（流水，支持撤销）
{ roomId, type, note, entries: [{ memberId, name, delta }], byOpenid, undone, createdAt }
```

- 余额用 `db.command.inc` **原子累加**，多人同时记账不会互相覆盖
- 写操作全部走云函数；余额更新失败会回滚对应流水，不产生孤儿记录
- 前端用 `watch` 实时订阅房间，**任何人记账其他人立刻看到**

### 数据库集合（新增）

| 集合 | 用途 | 权限建议 |
|---|---|---|
| `rooms` | 房间与余额 | **所有用户可读**，仅创建者可写（watch 需要读权限） |
| `records` | 记账流水 | 所有用户可读，仅创建者可写 |

> 注意：`rooms` 若设为「仅创建者可读写」，房间外的人 watch 不到，实时同步会失效。写入仍只走云函数，安全不受影响。
